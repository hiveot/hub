...
func DirectoryReadAllTDs(hc transports.IClientConnection, limit int, offset int) (tDlist []string, err error) {
	var args = DirectoryReadAllTDsArgs{limit, offset}
	err = hc.SendRequest("invokeaction", DirectoryDThingID, DirectoryReadAllTDsMethod, &args, &tDlist)
	return
}

// DirectoryReadTD client method - Read TD.
// Return a JSON encoded TD document
func DirectoryReadTD(hc transports.IClientConnection, thingID string) (tD string, err error) {

	err = hc.SendRequest("invokeaction", DirectoryDThingID, DirectoryReadTDMethod, &thingID, &tD)
	return
}

// DirectoryRemoveTD client method - Remove TD.
// Remove a digital twin from the directory
func DirectoryRemoveTD(hc transports.IClientConnection, dThingID string) (err error) {

	err = hc.SendRequest("invokeaction", DirectoryDThingID, DirectoryRemoveTDMethod, &dThingID, nil)
	return
}

// DirectoryUpdateTD client method - Update TD.
// Update the Thing TD in the directory. For use by agents only.
func DirectoryUpdateTD(hc transports.IClientConnection, tD string) (err error) {

	err = hc.SendRequest("invokeaction", DirectoryDThingID, DirectoryUpdateTDMethod, &tD, nil)
	return
}
...
func NewHandleDirectoryAction(svc IDirectoryService) func(msg *transports.ThingMessage) (any, error) {
