...
func DirectoryReadAllTDs(td1 *td.TD, hc transports.IClientConnection, limit int, offset int) (tDlist []string, err error) {
	var args = DirectoryReadAllTDsArgs{limit, offset}
	f := td1.GetForm("invokeaction", DirectoryReadAllTDsMethod, hc.GetProtocolType())
	err = hc.SendRequest(f, DirectoryDThingID, DirectoryReadAllTDsMethod, &args, &tDlist)
	return
}

// DirectoryReadTD client method - Read TD.
// Return a JSON encoded TD document
func DirectoryReadTD(td1 *td.TD, hc transports.IClientConnection, thingID string) (tD string, err error) {

	f := td1.GetForm("invokeaction", DirectoryReadTDMethod, hc.GetProtocolType())
	err = hc.SendRequest(f, DirectoryDThingID, DirectoryReadTDMethod, &thingID, &tD)
	return
}

// DirectoryRemoveTD client method - Remove TD.
// Remove a digital twin from the directory
func DirectoryRemoveTD(td1 *td.TD, hc transports.IClientConnection, dThingID string) (err error) {

	f := td1.GetForm("invokeaction", DirectoryRemoveTDMethod, hc.GetProtocolType())
	err = hc.SendRequest(f, DirectoryDThingID, DirectoryRemoveTDMethod, &dThingID, nil)
	return
}

// DirectoryUpdateTD client method - Update TD.
// Update the Thing TD in the directory. For use by agents only.
func DirectoryUpdateTD(td1 *td.TD, hc transports.IClientConnection, tD string) (err error) {

	f := td1.GetForm("invokeaction", DirectoryUpdateTDMethod, hc.GetProtocolType())
	err = hc.SendRequest(f, DirectoryDThingID, DirectoryUpdateTDMethod, &tD, nil)
	return
}
...
func NewHandleDirectoryAction(svc IDirectoryService) func(msg *transports.ThingMessage) transports.RequestStatus {
