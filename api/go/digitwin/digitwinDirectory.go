...
const DirectoryReadDTDsMethod = "readDTDs"

// DirectoryReadDTDsArgs defines the arguments of the readDTDs function
// Read TDs - Read a batch of TD documents
type DirectoryReadDTDsArgs struct {
...
// DirectoryReadDTDs client method - Read TDs.
// Read a batch of TD documents
func DirectoryReadDTDs(hc hubclient.IHubClient, limit int, offset int) (tDlist []string, err error) {
	var args = DirectoryReadDTDsArgs{limit, offset}
	err = hc.Rpc(DirectoryDThingID, DirectoryReadDTDsMethod, &args, &tDlist)
	return
}
...
type IDirectoryService interface {
	...
	// Read a batch of TD documents
	// This returns a list of JSON encoded TD documents
	ReadDTDs(senderID string, args DirectoryReadDTDsArgs) (tDlist []string, err error)
...
func NewHandleDirectoryAction(svc IDirectoryService) func(consumerID, dThingID, name string, input any, messageID string) (string, any, error) {
	return func(consumerID, dThingID, actionName string, input any, messageID string) (string, any, error) {
		...
		case "readDTD":
			...
			break
		case "readDTDs":
			args := DirectoryReadDTDsArgs{}
			err = utils.DecodeAsObject(input, &args)
			if err == nil {
				output, err = svc.ReadDTDs(consumerID, args)
			} else {
				err = errors.New("bad function argument: " + err.Error())
			}
			break
