...
func UserGetProfile(td1 *td.TD, hc transports.IClientConnection) (resp ClientProfile, err error) {

	f := td1.GetForm("invokeaction", UserGetProfileMethod, hc.GetProtocolType())
	err = hc.SendRequest(f, UserDThingID, UserGetProfileMethod, nil, &resp)
	return
}

// UserLogin client method - Login.
// Login with password
func UserLogin(td1 *td.TD, hc transports.IClientConnection, clientID string, password string) (token string, err error) {
	var args = UserLoginArgs{clientID, password}
	f := td1.GetForm("invokeaction", UserLoginMethod, hc.GetProtocolType())
	err = hc.SendRequest(f, UserDThingID, UserLoginMethod, &args, &token)
	return
}

// UserLogout client method - Logout.
// Logout from all devices
func UserLogout(td1 *td.TD, hc transports.IClientConnection) (err error) {

	f := td1.GetForm("invokeaction", UserLogoutMethod, hc.GetProtocolType())
	err = hc.SendRequest(f, UserDThingID, UserLogoutMethod, nil, nil)
	return
}

// UserRefreshToken client method - Request a new auth token for the current client.
func UserRefreshToken(td1 *td.TD, hc transports.IClientConnection, clientID string, oldToken string) (token string, err error) {
	var args = UserRefreshTokenArgs{clientID, oldToken}
	f := td1.GetForm("invokeaction", UserRefreshTokenMethod, hc.GetProtocolType())
	err = hc.SendRequest(f, UserDThingID, UserRefreshTokenMethod, &args, &token)
	return
}

// UserUpdateName client method - Request changing the display name of the current client.
func UserUpdateName(td1 *td.TD, hc transports.IClientConnection, newName string) (err error) {

	f := td1.GetForm("invokeaction", UserUpdateNameMethod, hc.GetProtocolType())
	err = hc.SendRequest(f, UserDThingID, UserUpdateNameMethod, &newName, nil)
	return
}

// UserUpdatePassword client method - Update Password.
// Request changing the password of the current client
func UserUpdatePassword(td1 *td.TD, hc transports.IClientConnection, password string) (err error) {

	f := td1.GetForm("invokeaction", UserUpdatePasswordMethod, hc.GetProtocolType())
	err = hc.SendRequest(f, UserDThingID, UserUpdatePasswordMethod, &password, nil)
	return
}

// UserUpdatePubKey client method - Update Public Key.
// Request changing the public key on file of the current client.
func UserUpdatePubKey(td1 *td.TD, hc transports.IClientConnection, publicKeyPEM string) (err error) {

	f := td1.GetForm("invokeaction", UserUpdatePubKeyMethod, hc.GetProtocolType())
	err = hc.SendRequest(f, UserDThingID, UserUpdatePubKeyMethod, &publicKeyPEM, nil)
	return
}
