...
func UserGetProfile(hc transports.IClientConnection) (resp ClientProfile, err error) {

	err = hc.SendRequest("invokeaction", UserDThingID, UserGetProfileMethod, nil, &resp)
	return
}

// UserLogin client method - Login.
// Login with password
func UserLogin(hc transports.IClientConnection, clientID string, password string) (token string, err error) {
	var args = UserLoginArgs{clientID, password}
	err = hc.SendRequest("invokeaction", UserDThingID, UserLoginMethod, &args, &token)
	return
}

// UserLogout client method - Logout.
// Logout from all devices
func UserLogout(hc transports.IClientConnection) (err error) {

	err = hc.SendRequest("invokeaction", UserDThingID, UserLogoutMethod, nil, nil)
	return
}

// UserRefreshToken client method - Request a new auth token for the current client.
func UserRefreshToken(hc transports.IClientConnection, clientID string, oldToken string) (token string, err error) {
	var args = UserRefreshTokenArgs{clientID, oldToken}
	err = hc.SendRequest("invokeaction", UserDThingID, UserRefreshTokenMethod, &args, &token)
	return
}

// UserUpdateName client method - Request changing the display name of the current client.
func UserUpdateName(hc transports.IClientConnection, newName string) (err error) {

	err = hc.SendRequest("invokeaction", UserDThingID, UserUpdateNameMethod, &newName, nil)
	return
}

// UserUpdatePassword client method - Update Password.
// Request changing the password of the current client
func UserUpdatePassword(hc transports.IClientConnection, password string) (err error) {

	err = hc.SendRequest("invokeaction", UserDThingID, UserUpdatePasswordMethod, &password, nil)
	return
}

// UserUpdatePubKey client method - Update Public Key.
// Request changing the public key on file of the current client.
func UserUpdatePubKey(hc transports.IClientConnection, publicKeyPEM string) (err error) {

	err = hc.SendRequest("invokeaction", UserDThingID, UserUpdatePubKeyMethod, &publicKeyPEM, nil)
	return
}
...
// NewHandleUserAction returns an agent handler for Thing 'dtw:authn:user' actions.
//
...
func NewHandleUserAction(svc IUserService) func(msg *transports.ThingMessage) (any, error) {
