...
// AdminClient client for talking to the 'dtw:authz:admin' service
type AdminClient struct {
	dThingID string
	hc       hubclient.IHubClient
}

// GetClientRole client method - Get Client Role.
// Read the role of a client
func (svc *AdminClient) GetClientRole(clientID string) (role string, err error) {
	err = svc.hc.Rpc(svc.dThingID, AdminGetClientRoleMethod, &clientID, &role)
	return
}

// SetClientRole client method - Set Client Role.
// Update the role of a consumer
func (svc *AdminClient) SetClientRole(args AdminSetClientRoleArgs) (err error) {
	err = svc.hc.Rpc(svc.dThingID, AdminSetClientRoleMethod, &args, nil)
	return
}

// NewAdminClient creates a new client for invoking Authorization Administration Service methods.
func NewAdminClient(hc hubclient.IHubClient) *AdminClient {
	cl := AdminClient{
		hc:       hc,
		dThingID: "dtw:authz:admin",
	}
	return &cl
}
