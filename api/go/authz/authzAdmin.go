...
func AdminGetClientRole(hc transports.IClientConnection, clientID string) (role ClientRole, err error) {

	err = hc.SendRequest("invokeaction", AdminDThingID, AdminGetClientRoleMethod, &clientID, &role)
	return
}

// AdminSetClientRole client method - Set Client Role.
// Update the role of a consumer
func AdminSetClientRole(hc transports.IClientConnection, clientID string, role ClientRole) (err error) {
	var args = AdminSetClientRoleArgs{clientID, role}
	err = hc.SendRequest("invokeaction", AdminDThingID, AdminSetClientRoleMethod, &args, nil)
	return
}
