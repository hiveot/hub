...
func AdminGetClientRole(td1 *td.TD, hc transports.IClientConnection, clientID string) (role ClientRole, err error) {

	f := td1.GetForm("invokeaction", AdminGetClientRoleMethod, hc.GetProtocolType())
	err = hc.SendRequest(f, AdminDThingID, AdminGetClientRoleMethod, &clientID, &role)
	return
}

// AdminSetClientRole client method - Set Client Role.
// Update the role of a consumer
func AdminSetClientRole(td1 *td.TD, hc transports.IClientConnection, clientID string, role ClientRole) (err error) {
	var args = AdminSetClientRoleArgs{clientID, role}
	f := td1.GetForm("invokeaction", AdminSetClientRoleMethod, hc.GetProtocolType())
	err = hc.SendRequest(f, AdminDThingID, AdminSetClientRoleMethod, &args, nil)
	return
}
