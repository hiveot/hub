...
func (svc *AddHistory) encodeValue(senderID string, tv *transports.ThingValue) (storageKey string, data []byte) {
	var err error
	...
	if tv.AffordanceType == transports.AffordanceTypeAction {
		// TODO: actions subscriptions are currently not supported. This would be useful though.
		storageKey = storageKey + "/a"
	} else if tv.AffordanceType == transports.AffordanceTypeProperty {
...
func (svc *AddHistory) AddValue(senderID string, tv transports.ThingValue) error {
	slog.Info("AddValue",
...
func (svc *AddHistory) AddMessage(msg *transports.ResponseMessage) error {
	...
	case wot.OpInvokeAction:
		tv.AffordanceType = transports.AffordanceTypeAction
		return svc.AddValue(msg.SenderID, tv) // response of an action
	case wot.OpObserveProperty:
		tv.AffordanceType = transports.AffordanceTypeProperty
	...
	case wot.OpSubscribeEvent, wot.OpSubscribeAllEvents:
		tv.AffordanceType = transports.AffordanceTypeEvent
		return svc.AddValue(msg.SenderID, tv)
	default:
...
func (svc *AddHistory) validateValue(senderID string, tv *transports.ThingValue) (retained bool, err error) {
	if tv.ThingID == "" {
	...
	if senderID == "" && tv.AffordanceType == transports.AffordanceTypeAction {
