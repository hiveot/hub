...
func (srv *LatestPropertiesStore) LoadProps(thingID string) (found bool) {
	srv.cacheMux.Lock()
...
func (srv *LatestPropertiesStore) GetProperties(thingAddr string, names []string) (props things.ThingMessageMap) {
	props = things.NewThingMessageMap()
...
func (srv *LatestPropertiesStore) HandleAddValue(addtv *things.ThingMessage) {
	// ensure the Thing has its properties cache loaded
	if addtv.CreatedMSec <= 0 {
		addtv.CreatedMSec = time.Now().UnixMilli()
	}

	srv.LoadProps(addtv.ThingID)
	srv.cacheMux.Lock()
	defer srv.cacheMux.Unlock()
	thingCache, _ := srv.cache[addtv.ThingID]

	if addtv.Key == vocab.EventTypeProperties {
	...
	srv.changedThings[addtv.ThingID] = true
}
