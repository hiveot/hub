...
import (
	"encoding/json"
	"fmt"
	"github.com/hiveot/hub/cmd/tdd2api/src/td2go"
...
// Td2GoCommand is a golang API generator from TD documents
func Td2GoCommand(apiDir string) *cli.Command {
	...
	return &cli.Command{
		...
		Action: func(cCtx *cli.Context) error {
			...
			err := HandleTd2Go(sourceDir, outDir)
			return err
		},
	}
}
...
// HandleTd2Go generates a go API for the .JSON sources
func HandleTd2Go(sourceDir string, outDirBase string) error {
	...
	for _, fullPath := range sourceFiles {
		...
		if err == nil {
			err = jsoniter.Unmarshal(tdJSON, &td1)
			if err != nil {
		...
		if td1.AtContext == nil {
			...
			l := &utils.SL{}
			idTitle := td2go.ToTitle(typeName)
			l.Add("package " + packageName)
			l.Add("")
			td2go.GenSchemaDefStruct(l, sourceNoExt, idTitle, &ds)
			err = l.Write(outFile)
		...
		if err == nil {
			...
			err = td2go.GenGoAPIFromTD(&td1, outFile)
		}
		if err == nil {
		...
		if err != nil {
			...
			fmt.Printf("%-24.24s  %9d  %-26.26s  %-30.30s  %-40.40s  %s\n",
				sourceFile, sizeKb, td1.ID, td1.Title, outFile, outputStatus)
		}
	}
	return nil
}
