...
func (svc *WotWssMessageConverter) DecodeRequest(raw []byte) (req *transports.RequestMessage) {
...
func (svc *WotWssMessageConverter) DecodeResponse(raw []byte) (resp *transports.ResponseMessage) {
	...
	case MsgTypeEvent: // Message type for event?
		wssMsg := EventMessage{}
		_ = svc.Unmarshal(raw, &wssMsg)
		resp = transports.NewResponseMessage(wot.OpSubscribeEvent,
