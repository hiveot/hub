...
func (c *WssServerConnection) onConnection(connected bool, err error) {
	...
	if c.connectionHandler != nil {
		c.connectionHandler(connected, err, c)
	}
}
...
func (c *WssServerConnection) onMessage(raw []byte) {
	...
	req, resp, err := c.messageConverter.ProtocolToHiveot(raw)
	if req != nil {
...
func (c *WssServerConnection) SendRequest(req *transports.RequestMessage) error {
	msg, err := c.messageConverter.RequestToProtocol(req)
	if err == nil {
		err = c._send(msg)
	}
	return err
...
func (c *WssServerConnection) SendResponse(resp *transports.ResponseMessage) (err error) {
	...
	msg, err := c.messageConverter.ResponseToProtocol(resp)
	if err == nil {
		err = c._send(msg)
	}
	return err
...
func (c *WssServerConnection) SetConnectHandler(h transports.ConnectionHandler) {
	c.connectionHandler = h
}
func (c *WssServerConnection) SetRequestHandler(h transports.RequestHandler) {
	c.requestHandler = h
}
func (c *WssServerConnection) SetResponseHandler(h transports.ResponseHandler) {
	c.responseHandler = h
}
