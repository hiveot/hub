...
// Paths used by this protocol binding - SYNC with HttpTransportClient.ts
//
...
func (cl *WssClientConnection) ConnectWithPassword(password string) (newToken string, err error) {
	...
	loginURL := fmt.Sprintf("https://%s%s", cl.BaseHostPort, PostLoginPath)
...
func (cl *WssClientConnection) ConnectWithToken(token string) (newToken string, err error) {
...
func (cl *WssClientConnection) RefreshToken(oldToken string) (newToken string, err error) {
	// use the http endpoint to refresh the token
	refreshURL := fmt.Sprintf("https://%s%s", cl.BaseHostPort, PostRefreshPath)
...
func (cl *WssClientConnection) Logout() error {
	...
	logoutURL := fmt.Sprintf("https://%s%s", cl.BaseHostPort, PostLogoutPath)
