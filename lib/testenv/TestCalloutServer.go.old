...
func (svr *TestCalloutServer) createNewAuthToken(clientPub string, clientID string) (newToken string, err error) {
	...
	uc.Audience = svr.appAccountName
...
func (svr *TestCalloutServer) Start() (clientURL string, err error) {
	...
	svr.appAccountName = "AppAccount"
	appAccount := server.NewAccount(svr.appAccountName)
	...
	if svr.caCert != nil && svr.serverCert != nil {
		caCertPool := x509.NewCertPool()
		caCertPool.AddCert(svr.caCert)
		...
		opts.TLSTimeout = 1000 // for debugging auth
	...
	//l := logger.NewStdLogger(true, true, false, true, true)
	//ns.SetLoggerV2(l, true, false, false)
	//ns.Logger().Warnf("test logging message")
