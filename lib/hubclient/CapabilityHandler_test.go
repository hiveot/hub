...
func Method1Ref(clientID string, args *M1Args) (*M1Res, error) {
	//slog.Info("Method1 called.", "p1", args.P1)
	res := M1Res{R1: args.P1}
	return &res, nil
}
func Method1Val(clientID string, args M1Args) (M1Res, error) {
	slog.Info("Method1 called.", "p1", args.P1)
	res := M1Res{R1: args.P1}
	return res, nil
}
...
func Method3ErrorResult(clientID string) error {
	slog.Info("Method3 called, returning error")
	return fmt.Errorf("method3 returns an error")
}
func Method4DataAndErrorResult(clientID string) (M1Res, error) {
	res := M1Res{R1: "data and err"}
	return res, fmt.Errorf("data and error")
}
func Method5StringArg(clientID string, arg1 string) (string, error) {
	slog.Info("received string arg", "arg1", arg1)
	return arg1, nil
}
func Method6IntArg(clientID string, arg1 int) (int, error) {
	slog.Info("received int arg", "arg1", arg1)
	return arg1, nil
}
func Method7ByteArrayArg(clientID string, arg1 []byte) ([]byte, error) {
	slog.Info("received array arg", "arg1", arg1)
	return arg1, nil
}
func Method8TwoArgsFail(clientID string, arg1 string, args2 string) (string, error) {
	slog.Info("received 2 args doesn't work", "arg1", arg1)
	return arg1, nil
}
func Method9ThreeResFail(clientID string, arg1 string) (string, string, error) {
...
func TestTwoArgsFail(t *testing.T) {
	...
	data, err := hubclient.HandleRequestMessage(senderID, Method8TwoArgsFail, sargJson)
	assert.Error(t, err)
	assert.Nil(t, data)
}
func TestThreeResFail(t *testing.T) {
	...
	data, err := hubclient.HandleRequestMessage(senderID, Method9ThreeResFail, sargJson)
	assert.Error(t, err)
	assert.Nil(t, data)
}
func Benchmark_Overhead(b *testing.B) {
	...
	b.Run("direct call, no marshalling",
		func(b *testing.B) {
			for n := 0; n < b.N; n++ {
				// pass args by reference
				m1res, err := Method1Ref(senderID, &m1args)
				_ = err
				_ = m1res
			}
		})
