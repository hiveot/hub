...
func (agent *DigitwinAgent) HandleAction(
	...
	if dThingID == digitwin.DirectoryDThingID {
		status, output, err = agent.dirHandler(consumerID, dThingID, actionName, input, requestID)
	} else if dThingID == digitwin.ValuesDThingID {
		status, output, err = agent.valuesHandler(consumerID, dThingID, actionName, input, requestID)
	} else {
