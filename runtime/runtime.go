...
import (
	...
	digitwin "github.com/hiveot/hivekit/go/modules/digitwin"
	...
	router "github.com/hiveot/hivekit/go/modules/router"
...
var recipe = factorypkg.FactoryRecipe{
	// modules used in the recipe in alpha order
	ModuleDefs: map[string]factory.ModuleDefinition{
		...
		// router module for connecting to agents and devices
		router.RouterModuleType: {
			Constructor: routerpkg.NewRouterServiceFactory,
		},
		// http server module is used by transports and discovery
	...
	ModuleChain: []string{
		// 1. initialize the environment with certificates - run-once
		certs.InitFactoryCertsModuleType,
		// 2. run a websocket server (this loads the http transport)
		wss.WotWebsocketServerModuleType,
		// 3. add forms to the published TDs
		addforms.AddFormsModuleType,
		// 4. run discovery server to publish the directory
		discovery.DiscoveryServerModuleType,
		// 5. run directory server to collect and publish TDs
		// * agents publish the TD of their devices to the hub
		// * TDs can be manually uploaded to the directory import folder (todo)
		// * the digitwin service hooks into the directory to convert device TDs to digital twins
		directory.DirectoryModuleType,
		// 6. The digitwin server module
		// * TDs of devices written to the directory are converted to digital twins
		// * Non-device TDs are not convert
		// * Read requests for digital twins are handled by the digitwin vcache
		// * Other requests for digital twins are converted and forwarded to the actual device
		digitwin.DigitwinModuleType,
		// ? the router forwards Thing requests to agents or devices
		// * this uses the directory to determine where the Thing lives and how to connect to it
		// * requests to Things served by agents are forwarded to the corresponding transport server
		// * requests to network devices are connected to on demand
		router.RouterModuleType,
	},
}
...
// 		utils.SetLogging(env.LogLevel, runtimeLogfileName)
// 	}
...
// func (r *Runtime) SendNotification(notif *msg.NotificationMessage) {
...
func NewRuntime(env *factory.AppEnvironment) *Runtime {
	r := &Runtime{env: env}
	return r
}
